 ...
 * - 'walk': Active walking state with alternating legs and bobbing
...
export function PixelPetSprite({
  ...
  positionX = 42, // horizontal translation offset in CSS px
  directionScale = -1, // 1 = right, -1 = left (flipped)
  isWalking = false,
  characterRef = null,
}) {
  ...
  const walkBob = isWalking && walkFrame === 1 ? -0.6 : 0;

  // Alternating leg offsets: front moves forward while back moves backward, then reverse
  const frontLegOffset = isWalking ? (walkFrame === 1 ? 2.5 : -2.5) : 0;
  const backLegOffset = isWalking ? (walkFrame === 1 ? -2.5 : 2.5) : 0;
  ...
  return (
    <div className="pixel-pet-stage">
      ...
      {/* ========================================================= */}
      {/* 2. OUTERMOST TURTLE WRAPPER (Translates & Flips Character) */}
      {/* ========================================================= */}
      <div
        ref={characterRef}
        className={`pixel-turtle-wrapper ${isWalking ? 'is-walking' : ''} state-${state}`}
        style={{
          transform: `translateX(${positionX}px) scaleX(${directionScale})`,
        }}
      >
        <svg
          viewBox="0 0 54 26"
          className={`turtle-sprite-svg pixel-turtle-svg dir-${lookDirection}`}
          shapeRendering="crispEdges"
          aria-hidden="true"
        >
          {/* Base orientation: oriented facing right so scaleX(1) is right, scaleX(-1) is left */}
          <g
            className="turtle-base"
            transform="translate(31, 0) scale(-1, 1)"
          >
            <g
              className={`turtle-creature ${isWalking ? 'is-walking' : ''}`}
              transform={walkBob !== 0 ? `translate(0, ${walkBob})` : undefined}
            >
              ...
              {/* Back Leg with visible alternating stride */}
              <g
                className="turtle-leg-back"
                transform={backLegOffset !== 0 ? `translate(${backLegOffset}, 0)` : undefined}
              >
                <rect x="21" y="17" width="3" height="4" fill="#16a34a" />
                <rect x="21" y="17" width="1" height="4" fill="#15803d" />
                <rect x="20" y="20" width="4" height="1" fill="#15803d" />
              </g>

              {/* Front Leg with visible alternating stride */}
              {!isLaptop && (
                <g
                  className="turtle-leg-front"
                  transform={frontLegOffset !== 0 ? `translate(${frontLegOffset}, 0)` : undefined}
                >
                  <rect x="9" y="17" width="3" height="4" fill="#22c55e" />
                  <rect x="11" y="17" width="1" height="4" fill="#16a34a" />
                  <rect x="8" y="20" width="4" height="1" fill="#16a34a" />
                  <rect x="8" y="20" width="2" height="1" fill="#4ade80" />
                </g>
              )}
